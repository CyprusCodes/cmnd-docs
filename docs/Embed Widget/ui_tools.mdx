...
There are 4 variations of UI tools that you can implement and use. 

1. UI tool that is rendered in the chat box as a message card which doesn't capture user input. These tools have to use the `ref` and `captureResults` parameters within their `runCmd` method. e.g: `plot_graph`
2. UI tool that is rendered in the chat box as a message card which captures user input. These tools have to use the `ref` parameter within their `runCmd` method. e.g: `capture_signature`
3. UI tool that is NOT rendered in the chat box which doesn't capture user input. These tools might access DOM directly if necessary. These tools only use the `functionArgs` paramter within `runCmd` method. e.g: `change_background_color`
4. UI tool that is NOT rendered in the chat box which captures user input. These tools might access DOM directly if necessary. These tools use the `functionArgs` and `captureResults` paramters within their `runCmd` method. e.g: `show_input_modal`
