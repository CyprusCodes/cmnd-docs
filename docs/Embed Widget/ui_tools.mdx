...
The built-in tools are preconfigured utilities that come with the embed widget. These tools are available by default unless you provide custom tools.
...
Custom tools allow you to extend the functionality of the embed widget by defining your own tools tailored to specific use cases. These tools can override the default built-in tools or work alongside them by leveraging `window.cmndBuiltInTools`. Custom tools provide flexibility and adaptability, enabling you to create unique experiences that align with your application's requirements.

:::warning
Providing custom tools overwites the built in tools, but you can extend your custom ui tools with the built in tools using `window.cmndBuiltInTools`.
:::
...
3. **`category`** and **`subcategory`**  
   Logical groupings for the tool.  
   Example:
...
4. **`functionType`**  
   Must always be set to `"ui"` for custom tools.

5. **`dangerous`**  
   Indicates whether the tool performs potentially harmful actions.  
   Example: `false`

6. **`associatedCommands`** and **`prerequisites`**  
...
By Default we do not need to register built in tools, but we need to register custom UI tools using the `registerTools` command
